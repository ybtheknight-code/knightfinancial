        ...
        <Card className="mb-8">
          <div className="flex flex-col md:flex-row items-center md:items-start gap-6">
            <div className={`w-24 h-24 rounded-full flex items-center justify-center text-4xl font-bold ${
              profile.is_prime ? 'bg-gradient-to-r from-knight-gold to-yellow-500 text-black' : 'bg-knight-gold-dark text-white'
            }`}>
              {getAvatar()}
            </div>
            <div className="flex-1 text-center md:text-left">
              <div className="flex items-center justify-center md:justify-start gap-3 mb-2">
                ...
                {profile.is_prime && (
                  <span className="bg-gradient-to-r from-knight-gold to-yellow-500 text-black px-3 py-1 rounded-full text-sm font-bold">⭐ PRIME</span>
                )}
              </div>
            ...
            {isOwnProfile && (
              <Link href="/settings" className="btn-knight-outline">⚙️ Edit Profile</Link>
            )}
          </div>
        </Card>
          ...
          <Card className="mb-8">
            ...
            <div className="flex flex-wrap gap-3">
              {profile.badges.map((badge: string, i: number) => (
                <span key={i} className="bg-knight-hover px-4 py-2 rounded-full text-white">{badge}</span>
              ))}
            </div>
          </Card>
        )}
        ...
        <Card>
          ...
          {posts && posts.length > 0 ? (
            <div className="space-y-3">
              {posts.map((post) => (
                <Link key={post.id} href={`/community/post/${post.id}`} className="block bg-knight-hover rounded-lg p-4 hover:bg-knight-gold-dark transition">
          ...
          {posts && posts.length > 0 && (
            <div className="text-center mt-4">
              <Link href={`/community?author=${id}`} className="text-knight-gold hover:underline">View all posts →</Link>
            </div>
          )}
        </Card>
      </div>
